...
`delete`, `head`, `options` and `trace` methods with identical signatures. They
...
The `httpur.auth` namespace contains the `basic` function for basic authentication.
It accepts a realm, user and a password and returns a function that can be applied
to a request for adding the basic auth headers:
...
* No automatic generation of query strings. There is no consensus on how to use
  query params for things like collection and don't want to make the choice for you.
